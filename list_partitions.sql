...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type pt, pg_namespace pn WHERE pn.oid=pt.typnamespace AND pt.typname='partition_list' AND nspname='c_partition') THEN
        CREATE TYPE c_partition.partition_list AS (table_schema text, partition_table text, partition_column text, column_type text, partition_type text, partition_method text);
    END IF;
END$$;

CREATE OR REPLACE FUNCTION c_partition.list_partitions() RETURNS SETOF c_partition.partition_list AS 
$function$
DECLARE
